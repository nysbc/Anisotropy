...
def threshold_binarize(inmrc, thresholded, thresholdedbinarized, FSCCutoff, ThresholdForSphericity, highpassfilter, apix):
        ...
        print("sum(sum(sum(outarraythresholdedbinarized2))) = %.5f"% (sum(sum(sum(outarraythresholdedbinarized2)))))
        print("sum(sum(sum(outarraythresholdedbinarized))) = %.5f"% (sum(sum(sum(outarraythresholdedbinarized)))))
        f1 = outarraythresholdedbinarized.flatten()
        f2 = outarraythresholdedbinarized2.flatten()
