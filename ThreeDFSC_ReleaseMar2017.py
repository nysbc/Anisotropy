...
#                 HalfMap1.hdf    HalfMap2.hdf   OutputStringLabel   A/pixel                    
...
# For right now, scroll down past the function definitions
# to section 0, which is the definition of variables
# modify by hand the lines
...
@autojit
def CreateFTLikeOutputs(inc,nx,ny,nz,ToBeAveraged):# created from CreateFSCOutputs
...
def AveragesOnShellsUsingLogicB(inc,retofRR,retofRI,n1ofR,n2ofR, kXofR,kYofR,kZofR, \
    ...
    for r in range(1,RMax+1):#range(1,inc+1):
        ...
        for jLoop in range(NumLoops):
            ...
            NumAtROutPre = AveragesOnShellsInnerLogicKernelnonCuda(kXNow,kYNow,kZNow, \
                                NumOnSurf, Thresh,Start, End);
            ...
            startTime = time.time()
            [retofROutRPre, retofROutIPre, n1ofROutPre,n2ofROutPre] = \
                   AveragesOnShellsInnerLogicC(retNowR,retNowI,n1Now, n2Now, Start, End,NumAtROutPre)
...
aa= np.abs(np.array(resultAve))<.05
bb=np.where(aa)[0];
try:
    RMax=bb[0]+4
except:
...
[kXofR,kYofR,kZofR,retofRR,retofRI,n1ofR,n2ofR,NumAtEachR] = \
    createFSCarrays(nx,ny,nz,lsd2,lr,inc,dx2,dy2,dz2,d1,d2)

[kXofR,kYofR,kZofR,retcH,retFT,n12ofR] = \
    createFTarrays(nx,ny,nz,lsd2,lr,inc,dx2,dy2,dz2,dcH,dFPower)
...
h5f_write = h5py.File(ResEMOutHDF_FN,'w')
